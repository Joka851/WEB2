...
export const checklistService = {
  getAll: async (travelPlanId: number): Promise<ChecklistItem[]> => {
    const response = await axios.get(
      `${API_URL}/api/travel-plans/${travelPlanId}/checklist`,
      getHeaders()
    );
  ...
  toggle: async (travelPlanId: number, id: number): Promise<ChecklistItem> => {
    const response = await axios.put(
      `${API_URL}/api/travel-plans/${travelPlanId}/checklist/${id}/toggle`,
      {},
      getHeaders()
    );
    return response.data;
  ...
  delete: async (travelPlanId: number, id: number): Promise<void> => {
    await axios.delete(
      `${API_URL}/api/travel-plans/${travelPlanId}/checklist/${id}`,
      getHeaders()
    );
  }
};